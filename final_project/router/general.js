...
public_users.get('/isbn/:isbn', function (req, res) {
  ...
  for (bookISBN in books){
    if(bookISBN === ISBN){
      book.push({'author': books[bookISBN].author, 'title': books[bookISBN].title});
    }
  }
  ...
  if(book.length > 0){
    ...
    return res.status(404).json({'message': `The book with ISBN ${ISBN} doesn't exist`})
  }
})
...
public_users.get('/author/:author', function (req, res) {
  ...
  }else{ // The author doesn't exist
    // Notify the client author doesn't exist
    return res.status(404).json({'message':`¡The author ${author} doesn't exist!`})
  }
})
...
public_users.get('/title/:title', function (req, res) {
  ...
  if(bookList.length > 0){
    ...
    return res.status(404).json({'message':`¡Book with title ${title} doesn't exist!`})
  }
});
